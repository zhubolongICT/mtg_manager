...
def download2File(url, dirpath, filename):
    ...
    print('save-filepath:', filepath, 'dirpath:', dirpath, 'filename:', filename)
...
def downloadCheckList(setId, lang='en'):
    checkListUrl = "https://scryfall.com/sets/" + setId + "?as=checklist"
    if lang != 'en':
        checkListUrl = "https://scryfall.com/sets/" + setId + "/" + lang + "?as=checklist"

    r = requests.get(checkListUrl, headers=headers)
    soup = BeautifulSoup(r.text, 'html.parser')

    rsList = list()
    for tr in soup.find_all('tr'):
        attrs = tr.attrs
        if "data-component" in attrs and attrs["data-component"] is not None:
                if attrs["data-component"] == "card-tooltip":
                    if "data-card-id" in attrs:
                        # '/card/bro/1/aeronaut-cavalry'
                        card_href = tr.td.a['href']
                        pos = card_href.rfind('/')
                        card_id = card_href[6:pos]
                        info_url = 'https://api.scryfall.com/cards/'+card_id+'?format=json&pretty=true'
                        rsList.append({
                            'card_href': card_href,
                            'card_id': card_href[6:pos],
...
def downloadPNGs(info_dirpath, png_dirpath, error_list):
    for filename in os.listdir(info_dirpath):
        if not filename.endswith('.json'):
            continue
        filepath = os.path.join(info_dirpath, filename)
            ...
            try:
                ...
                if 'image_uris' not in jobj:
                    if 'card_faces' in jobj:
                        ...
                        download2File(img_url_front, png_dirpath, output_filename)
                        time.sleep(0.5)

                        img_url_back = jobj['card_faces'][1]['image_uris']['png']
                        output_filename = "%s_%s_back.png" % (collection_set, collector_number)
                        download2File(img_url_back, png_dirpath, output_filename)
                        time.sleep(0.5)
                else:
                ...
                error_list.append('{"type": "png", "info_json_filepath": "%s"}' % filepath)
                print(e.args)
    pass


def downloadInfos(error_list, info_dirpath, setId, lang='en'):
    cardInfoList = downloadCheckList(setId, lang)
    for card in cardInfoList:
        ...
        info_url = info_url_template % card['card_id']
        try:
            download2File(info_url, info_dirpath, '%s.json' % uniq_card_id)
            time.sleep(0.5)
        except Exception as e:
            error_list.append('{"type": "info", "card_id": "%s", "info_url": "%s"}' % (card['card_id'], info_url))
            print(e.args)


def downloadOneSet(setConfig, rootDirpath:str='../mtgcards'):
    error_list = list()

    setId = setConfig['SetId']
    langs = setConfig['Lang']

    for lang in langs:
        info_dirpath = os.path.join(rootDirpath, "%s/%s/info" % (setId, lang))
        png_dirpath = os.path.join(rootDirpath, "%s/%s/png" % (setId, lang))

        if not os.path.exists(info_dirpath):
            os.makedirs(info_dirpath)
        if not os.path.exists(png_dirpath):
            os.makedirs(png_dirpath)

        downloadInfos(error_list, info_dirpath, setId, lang)
        downloadPNGs(info_dirpath, png_dirpath, error_list)

    error_list_filepath = os.path.join(rootDirpath, "%s/error_list.txt" % setId)
    with codecs.open(error_list_filepath, mode='w', encoding='utf-8') as fp:
...
def main():
    for setConfig in toBeDownloadSets:
        downloadOneSet(setConfig, '../mtgcards')
