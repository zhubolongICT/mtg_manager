...
def downloadOneSet(setConfig):
    ...
    for lang in langs:
        info_dirpath = "%s/%s/info" % (setId, lang)
        png_dirpath = "%s/%s/png" % (setId, lang)

        os.system("mkdir -p %s" % info_dirpath)
        os.system("mkdir -p %s" % png_dirpath)
